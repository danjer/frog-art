...
const mockDetails = (urls) => {
    return urls.map((url, index) => ({
        url: url,
        name: `Comparable Artwork No. ${index + 1}`,
        artist: 'Jan Lul',
        purchaseLink: `Purchase Link: ${url}`
    }));
};
...
export default function ResultScreen() {
    ...
    try {
        if (params.imageUrls) {
            const urls = JSON.parse(params.imageUrls);
            parsedImages = mockDetails(urls); // enrich the data with mock data
        }
    } catch (e) {
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.header}>
                <Text style={styles.title}>Results ({activeIndex + 1}/{parsedImages.length})</Text>
            </View>
            ...
            {parsedImages.length > 0 ? (
                <>
                    ...
                    <View style={styles.detailsContainer}>
                        ...
                        <Text style={[styles.detailText, styles.linkText]}>{activeDetails.purchaseLink}</Text>
                    </View>
                </>
            ) : (
